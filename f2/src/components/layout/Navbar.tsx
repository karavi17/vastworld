...
export const Navbar = () => {
  ...
  const languages: Language[] = ['RANDOM', 'HINDI', 'ENGLISH', 'TAMIL', 'TELUGU', 'KANNADA', 'MALAYALAM', 'BENGALI', 'PUNJABI', 'MARATHI'];

  return (
    <nav className="fixed top-0 left-0 right-0 h-14 bg-[#0f0f0f] flex items-center justify-between px-2 sm:px-4 z-50 border-b border-[#303030]">
      {/* Mobile Search Overlay */}
      {isSearchVisible && (
        <div className="absolute inset-0 bg-[#0f0f0f] flex items-center px-2 z-50">
          <button 
      ...
      <div className={`flex items-center gap-1 sm:gap-4 ${isSearchVisible ? 'invisible' : ''}`}>
        ...
        <Link to="/" className="flex items-center gap-2">
          <img src="/logo.png" alt="Logo" className="h-6 sm:h-8 w-auto object-contain" />
      ...
      <div className={`flex items-center gap-0.5 sm:gap-2 ${isSearchVisible ? 'invisible' : ''}`}>
        ...
        <div className="relative">
          ...
          {showLangMenu && (
            <div className="absolute right-0 mt-2 w-40 bg-[#282828] border border-[#404040] rounded-xl shadow-2xl py-2 z-50">
