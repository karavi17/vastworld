  ...
  useEffect(() => {
    const fetchVideoData = async () => {
      ...
      try {
        ...
        if (isInitialLoad) {
          if (infoResult && infoResult.status === 'fulfilled' && infoResult.value.status === 'success' && infoResult.value.data.subject) {
            const currentSubject = (infoResult as PromiseFulfilledResult<any>).value.data.subject;
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mt-2 pb-4 border-b border-[#303030]">
            ...
            <div className="flex items-center gap-2">
              ...
              <button 
                onClick={handleShare}
                className="flex items-center gap-2 bg-[#272727] text-white px-4 py-2 rounded-full text-sm font-medium hover:bg-[#3f3f3f] transition-colors"
              >
                <Share2 size={18} />
                Share
              </button>
              ...
              <div className="relative" ref={menuRef}>
                <button 
                  ...
                  className={`p-2 bg-[#272727] text-white rounded-full hover:bg-[#3f3f3f] transition-colors ${isMenuOpen ? 'bg-[#3f3f3f]' : ''}`}
                >
                  <MoreVertical size={18} />
                </button>
                
                {isMenuOpen && (
                  <div className="absolute right-0 bottom-full mb-2 w-48 bg-[#282828] border border-white/10 rounded-xl shadow-2xl py-2 z-50 animate-in fade-in zoom-in duration-100">
                    <button 
                      onClick={() => {
                        toggleWatchLater();
                        setIsMenuOpen(false);
                      }}
                      className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-white/10 text-white transition-colors text-sm"
                    >
                      {isWatchLater ? (
                        <>
