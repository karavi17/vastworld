...
async function handleRelated(movieId) {
    // Try recommendations first
    const params = new URLSearchParams({ subjectId: movieId });
    let response = await makeApiRequest(`${HOST_URL}/wefeed-h5-bff/web/subject/recommend?${params}`);
    ...
    if (!content || (!content.items && !content.subjectList) || (content.items?.length === 0 && content.subjectList?.length === 0)) {
        const infoRes = await makeApiRequest(`${HOST_URL}/wefeed-h5-bff/web/subject/detail?${params}`);
        ...
        if (title) {
            ...
            const searchRes = await makeApiRequest(`${HOST_URL}/wefeed-h5-bff/web/subject/search`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(searchPayload)
            });
            const searchData = await searchRes.json();
            content = processApiResponse(searchData);
        }
    }
    ...
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            if (url.pathname.startsWith('/api/related/')) {
                const movieId = url.pathname.split('/api/related/')[1];
                return await handleRelated(movieId);
            }
